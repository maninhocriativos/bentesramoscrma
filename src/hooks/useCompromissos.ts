    ...
    <Collapsible open={open} onOpenChange={setOpen}>
      <div className="border border-border/60 rounded-lg bg-card">
        <CollapsibleTrigger asChild>
          <button type="button" className="w-full flex items-center justify-between px-4 py-3 hover:bg-muted/30 transition-colors rounded-lg">
...
export function CompromissoModal({ isOpen, onClose, compromisso, selectedDate }: CompromissoModalProps) {
  ...
  useEffect(() => {
    if (compromisso) {
      ...
      // ✅ FIX: quando abre via "Novo" sem selectedDate, garantir data hoje
      const dataManaus = toZonedTime(new Date(), MANAUS_TZ);
      form.reset({
        titulo: '',
        descricao: '',
        data_inicio: format(dataManaus, 'yyyy-MM-dd'),
  ...
  }, [compromisso, selectedDate, isOpen, isNew, form]);

  const onSubmit = async (data: FormData) => {
    console.log('[CompromissoModal] onSubmit chamado com:', data);
    setSubmitting(true);

    try {
      const dataHoraLocal = `${data.data_inicio}T${data.hora_inicio || '09:00'}:00`;
      const dataUtc = fromZonedTime(dataHoraLocal, MANAUS_TZ);
      console.log('[CompromissoModal] Data UTC convertida:', dataUtc.toISOString());

      if (compromisso) {
        console.log('[CompromissoModal] Atualizando compromisso:', compromisso.id);
        ...
        console.log('[CompromissoModal] Update result:', result);
        if (result?.error) {
          console.error('[CompromissoModal] Erro no update:', result.error);
          return;
        }
      } else {
        console.log('[CompromissoModal] Criando novo compromisso...');
        ...
        console.log('[CompromissoModal] Create result:', result);
        if (result?.error) {
          console.error('[CompromissoModal] Erro na criação:', result.error);
          return;
        }
      }

      console.log('[CompromissoModal] Sucesso, fechando modal');
      onClose();
    } catch (err) {
      console.error('[CompromissoModal] Exceção em onSubmit:', err);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent
        ...
        onOpenAutoFocus={(e) => {
          // Evita erros de foco quando há submit
          e.preventDefault();
        }}
      >
        {/* ✅ FIX: DialogTitle e DialogDescription escondidos (acessibilidade) */}
        <DialogHeader className="sr-only">
          <DialogTitle>
            {isNew ? 'Novo Compromisso' : compromisso?.titulo || 'Compromisso'}
          </DialogTitle>
          <DialogDescription>
            {isNew
              ? 'Preencha os dados para criar um novo compromisso na agenda.'
              : 'Visualize ou edite os dados do compromisso.'}
          </DialogDescription>
        </DialogHeader>
        ...
        {!isNew && (
          <div className="flex items-center gap-2 px-6 py-3 border-b border-border/40 bg-muted/20">
            ...
            <Button type="button" variant="outline" size="sm" className="h-8 text-xs gap-1.5" onClick={() => setIsEditing(!isEditing)}>
              <Pencil className="h-3.5 w-3.5" />
              Editar
            </Button>
            <Button type="button" variant="outline" size="sm" className="h-8 text-xs gap-1.5" onClick={handleDelete}>
            ...
            <Form {...form}>
              <form
                ...
                onSubmit={form.handleSubmit(onSubmit, (errors) => {
                  console.error('[CompromissoModal] Validação falhou:', errors);
                })}
                className="space-y-4"
              >
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="compromisso-titulo" className="text-xs font-bold text-foreground">Título</FormLabel>
                      <FormControl>
                        <Input
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="compromisso-tipo" className="text-xs font-bold text-foreground">Tipo da Tarefa</FormLabel>
                      <Select onValueChange={field.onChange} value={field.value}>
                        <FormControl>
                          <SelectTrigger id="compromisso-tipo">
                            <SelectValue />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel htmlFor="compromisso-data" className="text-xs font-bold text-foreground">Data</FormLabel>
                        <FormControl>
                          <Input
                            id="compromisso-data"
                            type="date"
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel htmlFor="compromisso-hora" className="text-xs font-bold text-foreground">Hora</FormLabel>
                        <FormControl>
                          <Input
                            id="compromisso-hora"
                            type="time"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="compromisso-descricao" className="text-xs font-bold text-foreground">Descrição</FormLabel>
                      <FormControl>
                        <Textarea
                ...
                <div className="flex gap-2 pt-4 border-t border-border/40">
                  ...
                  >
                    {(submitting || form.formState.isSubmitting) && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}
                    Salvar
                  </Button>
                </div>
              </form>
