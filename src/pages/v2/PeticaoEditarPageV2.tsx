// Formulário dinâmico de preenchimento (v2 — backend Cloudflare). Os campos
// vêm dos {{marcadores}} reais do .docx do modelo (fetchModelFields), a
...
export default function PeticaoEditarPageV2() {
    ...
    const init = async () => {
      ...
      try {
        if (id) {
          ...
          if (!petition) { navigate('/peticoes-v2'); return; }
        ...
        if (actionId && modelId) {
          ...
          setPetitionId(newId);
          navigate(`/peticoes-v2/${newId}/editar`, { replace: true });
          return;
        }

        navigate('/peticoes-v2');
      } catch (err) {
  ...
  const doAutosave = useCallback(async () => {
    ...
    try {
      ...
      console.error('[PeticaoEditarPageV2] autosave falhou:', err);
    }
  ...
  const goPrev = () => {
    if (currentIdx === 0) navigate('/peticoes-v2');
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      navigate(`/peticoes-v2/${petitionId}/revisao`);
    } catch (err) {
      console.error(err);
              ...
              {isReviewStep && (
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-muted-foreground">
                    ...
                    O print do contrato ainda não é suportado nesta versão beta — se o modelo precisar dele, gere pela versão oficial em <button className="underline font-semibold" onClick={() => navigate('/peticoes')}>/peticoes</button>.
                  </p>
                </div>
              )}
            </CardContent>
