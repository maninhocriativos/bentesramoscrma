// Revisão + histórico de versões (v2 — backend Cloudflare). Sem prévia em
// PDF nesta versão beta (o Worker ainda não tem conversão docx-to-pdf) — os
// botões baixam o .docx direto do R2 via peticoesAuthBridge.
...
export default function PeticaoRevisaoPageV2() {
  const { id } = useParams();
    ...
    return (
      <><AppHeader title="Petição não encontrada" />
        <div className="flex-1 flex items-center justify-center flex-col gap-4">
          ...
          <Button onClick={() => navigate('/peticoes-v2')}>Voltar</Button>
        </div>
      </>
    );
  }
  ...
  return (
    <>
      <AppHeader title="Revisão da Petição (v2 — beta)" />
      <ScrollArea className="flex-1">
        <div className="p-4 md:p-6 max-w-[1000px] mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <Button variant="ghost" onClick={() => navigate('/peticoes-v2')} className="gap-2 rounded-xl">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" onClick={() => navigate(`/peticoes-v2/${id}/editar`)} className="gap-2 rounded-xl" size="sm">
