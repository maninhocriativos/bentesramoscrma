// Versão v2 (backend Cloudflare) da tela de Petições — reaproveita o design já
// validado de PeticoesPage.tsx (Supabase), só troca a fonte de dados. Rota
// paralela /peticoes-v2, não linkada na barra lateral ainda — a v1 continua
// sendo a oficial até a API real (Worker) estar pronta e testada ponta a ponta.
...
export default function PeticoesPageV2() {
  ...
  function handleOpen(id: string, status: string) {
    navigate(status === 'generated' || status === 'filed' ? `/peticoes-v2/${id}/revisao` : `/peticoes-v2/${id}/editar`);
  }
  ...
  return (
    <>
      <AppHeader title="Gerador de Petições (v2 — beta)" />
      <ScrollArea className="flex-1">
        <div className="p-4 md:p-6 space-y-5 max-w-[1680px] mx-auto">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_380px] gap-5 items-start">
            ...
            <div className="hidden xl:block xl:sticky xl:top-4" style={{ height: 'calc(100vh - 170px)' }}>
              <ModelsSidePanel actionTypes={actionTypes} getModelsForAction={getModelsForAction} onSelectModel={(actionId, modelId) => navigate(`/peticoes-v2/nova?action=${actionId}&model=${modelId}`)} />
            </div>
          </div>
        </div>
      </ScrollArea>
      ...
      <NovaPeticaoModal open={modalOpen} onClose={() => setModalOpen(false)} actionTypes={actionTypes} getModelsForAction={getModelsForAction}
        onConfirm={(actionId, modelId) => navigate(`/peticoes-v2/nova?action=${actionId}&model=${modelId}`)} />
    </>
  );
}
