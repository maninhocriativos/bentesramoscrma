...
const ManyChatInboxContent = () => {
  ...
  const { getTeamWithStatus, setCurrentChat, getOnlineCount } = useTeamPresence(
    user?.id,
    fullName || user?.email?.split("@")[0],
  );

  const { playNotificationSound, notifyAssignment, notifyNewMessage, requestNotificationPermission } =
  ...
  const getProviderMessageId = (msg: any): string | undefined => {
    const mid =
      ...
      msg?.metadata?.original?.messageId ||
      msg?.metadata?.original?.id?.id ||
      msg?.metadata?.original?.id;
  ...
  useEffect(() => {
    ...
    const setupMessagesChannel = () => {
      const channel = supabase
        ...
        .on("postgres_changes", { event: "INSERT", schema: "public", table: "manychat_mensagens" }, (payload) => {
    ...
    const setupSubscribersChannel = () => {
      const channel = supabase
        ...
        .on("postgres_changes", { event: "*", schema: "public", table: "manychat_subscribers" }, (payload) => {
          if (!isSubscribed) return;
  ...
  const loadSubscribers = async () => {
    ...
    try {
      ...
      if (subsWithoutInstance.length > 0) {
        ...
        console.log(
          "[loadSubscribers] Instance lookup: skipped",
          rawSubscribers.length - subsWithoutInstance.length,
          "subs (already have instance_name), queried for",
          subsWithoutInstance.length,
        );
      }
      ...
      for (const sub of rawSubscribers) {
        ...
        const subWithOrigem = {
          ...
          instance_name:
            sub.instance_name ||
            (sub.lead_id ? instanceByLeadId.get(sub.lead_id) : undefined) ||
            instanceBySubscriberId.get(sub.subscriber_id) ||
            undefined,
        };
        ...
        if (!existing) {
          ...
          const existingHasName =
            existing.nome && !existing.nome.includes("Desconhecido") && !existing.nome.includes("Contato");
      ...
      const uniqueSubscribers = Array.from(deduplicatedMap.values()).sort(
        (a, b) => new Date(b.ultima_interacao || 0).getTime() - new Date(a.ultima_interacao || 0).getTime(),
      );
        ...
        if (refreshedSelected) {
          ...
          setSelectedSubscriber((prev) =>
            prev
              ? {
                  ...prev,
                  instance_name: refreshedSelected.instance_name,
                  lead_tipo_origem: refreshedSelected.lead_tipo_origem,
                  nome: refreshedSelected.nome || prev.nome,
                  lead_id: refreshedSelected.lead_id || prev.lead_id,
                }
              : null,
          );
        }
  ...
  const loadMessages = async (subscriberId: string, loadAll = false, subscriberOverride?: Subscriber | null) => {
    ...
    try {
      ...
      const currentSub =
        subscriberOverride ||
        subscribers.find((s) => s.subscriber_id === subscriberId) ||
        selectedSubscriberRef.current;
      ...
      console.log("[loadMessages] Buscando mensagens para:", {
        subscriberId,
        possibleIds: idsArray,
        leadId,
        phoneClean,
      });
      ...
      console.log(
        "[loadMessages] Mensagens carregadas:",
        uniqueMessages.length,
        "entrada:",
        uniqueMessages.filter((m) => m.direcao === "entrada").length,
        "saída:",
        uniqueMessages.filter((m) => m.direcao === "saida").length,
      );
      ...
      if (selectedSubscriberRef.current?.subscriber_id !== subscriberId) {
        console.log(
          "[loadMessages] Resultado descartado - conversa mudou:",
          subscriberId,
          "→",
          selectedSubscriberRef.current?.subscriber_id,
        );
        return;
      }
      ...
      setMessages((prev) => {
        ...
        const tempMessages = prev.filter(
          (m) => m.id.startsWith("temp_") && (!m.subscriber_id || m.subscriber_id === subscriberId),
        );
        ...
        const realtimeOnly = prev.filter((m) => {
          ...
          const belongsToConversation =
            msgSubId === subscriberId || (leadId && (m as any).lead_id === leadId) || idsArray.includes(msgSubId);
  ...
  const assignConversation = async (memberId: string) => {
    ...
    try {
      ...
      setSubscribers((prev) =>
        prev.map((s) => (s.subscriber_id === selectedSubscriber.subscriber_id ? { ...s, assigned_to: memberId } : s)),
      );
    } catch (error) {
      ...
      try {
        ...
        setMessages((prev) => {
          const updated = prev.map((m) =>
            m.id === tempId ? { ...m, metadata: { ...((m as any).metadata || {}), send_error: true } } : m,
          );
          messagesCacheRef.current.set(subscriberSnapshot.subscriber_id, updated);
          return updated;
        });
  ...
  const sendAudioFromPreview = async () => {
    ...
    try {
      ...
      setMessages((prev) => {
        const updated = prev.map((m) =>
          m.id === tempId ? { ...m, conteudo: "❌ Erro no envio do áudio", metadata: { send_error: true } } : m,
        );
        messagesCacheRef.current.set(subscriberSnapshot.subscriber_id, updated);
        return updated;
      });
  ...
  const getDisplayName = (sub: Subscriber) => {
    ...
    const hasValidName =
      sub.nome && !invalidNames.includes(sub.nome) && !sub.nome.startsWith("{{") && !sub.nome.startsWith("[");
  ...
  const ChannelIcon = ({ canal, size = "sm" }: { canal?: string; size?: "sm" | "md" }) => {
    ...
    if (normalizedCanal.includes("instagram") || normalizedCanal === "ig") {
      return (
        <div
          className={`${size === "sm" ? "p-0.5" : "p-1"} rounded bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600`}
        >
  ...
  const loadMessageFlags = useCallback(
    ...
    async (messageId: string) => {
      ...
      if (providerMessageId && selectedSubscriber?.telefone) {
        ...
        if (error || !data?.success) {
          ...
          toast({
            title: "⚠️ Apagada localmente",
            description: "Não foi possível apagar no WhatsApp",
            variant: "destructive",
          });
        } else {
  ...
  const handleConfirmEdit = useCallback(async () => {
    ...
    if (providerMessageId && selectedSubscriber?.telefone) {
      ...
      if (error || !data?.success) {
        ...
        toast({
          title: "⚠️ Editada localmente",
          description: "Não foi possível editar no WhatsApp",
          variant: "destructive",
        });
      } else {
  ...
  const handleForwardToSubscribers = useCallback(
  ...
  const handleSendContact = useCallback(
    async (contact: { nome: string; telefone?: string; subscriber_id: string }) => {
  ...
  const renderMessage = (message: Message) => {
    ...
    if (isDocument) {
      ...
      if (isPdf) {
        ...
        return (
          <div
            className={`flex flex-col rounded-lg overflow-hidden ${isDark ? "bg-[#1F2C33]" : "bg-[#F0F2F5]"} max-w-[320px]`}
          >
            {/* Preview do PDF */}
      ...
      return (
        <div
          className={`flex items-center gap-3 p-3 rounded-lg ${isDark ? "bg-[#1F2C33]" : "bg-[#F0F2F5]"} min-w-[200px] max-w-[300px]`}
        >
  ...
  return (
    <div className={`flex h-dvh w-full overflow-hidden ${themeClasses.bg}`}>
      ...
      <div
        className={`${showMobileChat ? "hidden md:flex" : "flex"} w-full md:w-[440px] lg:w-[500px] xl:w-[540px] flex-col ${themeClasses.sidebar} border-r ${themeClasses.border}`}
      >
        {/* Header */}
        <div
          className={`h-[60px] px-4 flex items-center justify-between backdrop-blur-md ${isDark ? "bg-gradient-to-r from-[#202C33] to-[#1A252C]" : "bg-gradient-to-r from-[#F0F2F5] to-[#E8EBEE]"}`}
        >
                    ...
                    <div className="flex-1 min-w-0 grid grid-cols-[minmax(0,1fr)_auto] items-start gap-2 pr-1">
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 min-w-0 overflow-hidden">
                          <span
                            className={`text-[15px] truncate leading-tight font-medium ${isUnreadVisual ? "text-[#E9EDEF] font-semibold" : themeClasses.headerText}`}
                          >
                          ...
                          {instanceInfo ? (
                            ...
                            >
                              {formatPhone(subscriber.telefone) ? `${formatPhone(subscriber.telefone)} · ` : ""}
                              {instanceInfo.label}
                            </span>
                          ) : (
                            formatPhone(subscriber.telefone) && (
                              <span
                                className={`text-[10px] px-1.5 py-0.5 rounded shrink-0 leading-none ${themeClasses.secondaryText}`}
                              >
                                {formatPhone(subscriber.telefone)}
                              </span>
                            )
                        ...
                        <div className="flex items-center gap-1 min-w-0 mt-[3px] overflow-hidden">
                          ...
                          <p
                            className={`text-[13px] truncate leading-tight ${isUnreadVisual ? "text-[#D1D7DB] font-medium" : themeClasses.secondaryText}`}
                          >
                            {msgPreview
                              ? msgPreview.startsWith("Você: ")
                                ? msgPreview.slice(6)
                      ...
                      <div className="w-[76px] min-w-[76px] shrink-0 flex flex-col items-end justify-between gap-1">
                        <span
                          className={`text-[12px] leading-tight whitespace-nowrap text-right ${isUnreadVisual ? "text-[#25D366] font-semibold" : themeClasses.secondaryText}`}
                        >
      ...
      <div className={`${!showMobileChat ? "hidden md:flex" : "flex"} flex-1 flex-col min-w-0`}>
        {selectedSubscriber ? (
          <>
            {/* Header do Chat */}
            <div
              className={`h-[50px] md:min-h-[62px] px-1.5 md:px-4 flex items-center gap-1.5 md:gap-3 backdrop-blur-md border-b ${themeClasses.border} ${isDark ? "bg-gradient-to-r from-[#202C33] to-[#1A252C]" : "bg-gradient-to-r from-[#F0F2F5] to-[#E8EBEE]"}`}
            >
              <Button
                variant="ghost"
                size="icon"
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5 overflow-hidden max-h-[16px] md:max-h-none">
                  ...
                  {isTyping(selectedSubscriber.subscriber_id) && (
                    <span className="text-[11px] md:text-xs text-[#00A884] font-medium animate-pulse">
                      digitando...
                    </span>
                  )}
                  {/* Tags do contato - hidden on mobile to save space */}
                  <div className="hidden md:contents">
                    {getSubscriberTags(selectedSubscriber.subscriber_id)
                      .slice(0, 3)
                      .map(
                        (st) =>
                          st.tag && (
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      size="icon"
                      className={`h-8 w-8 md:h-10 md:w-10 rounded-full ${themeClasses.iconColor} ${themeClasses.hoverBtn}`}
                    >
                  ...
                  <DropdownMenuContent align="end" className="w-52">
                    ...
                    {selectedSubscriber.telefone && (
                      <>
                        ...
                        <DropdownMenuItem
                          className="md:hidden"
                          onClick={() => window.open(`tel:${selectedSubscriber.telefone}`, "_self")}
                        >
                          📞 Ligar
                        </DropdownMenuItem>
                      </>
            ...
            >
              {isLoadingMessages ? (
                ...
                <div className="h-full flex items-center justify-center">
                  <div
                    className={`${isDark ? "bg-[#332F24]" : "bg-[#FCF4CB]"} rounded-xl px-6 py-4 max-w-md shadow-lg`}
                  >
                    ...
                    .map((message, index, filteredMsgs) => {
                      ...
                      return (
                        <div
                          key={message.id}
                          id={`msg-${message.id}`}
                          className={`transition-colors duration-700 rounded-lg ${isHighlighted ? (isDark ? "bg-[#00A884]/10" : "bg-[#00A884]/08") : ""}`}
                        >
                          {dateLabel && (
                            <div className="flex justify-center my-4">
                              <span
                                className={`px-4 py-1.5 rounded-lg ${isDark ? "bg-[#1F2C34]" : "bg-white"} text-[12px] ${themeClasses.secondaryText} shadow-sm font-medium`}
                              >
                                {dateLabel}
                              </span>
                            </div>
                          ...
                          <div className={`flex ${isOutgoing ? "justify-end pr-2" : "justify-start pl-2"} mb-[3px]`}>
                            ...
                            >
                              ...
                              <MessageContextMenu
                                ...
                                isPinned={
                                  selectedSubscriber
                                    ? pinnedMessagesBySubscriber[selectedSubscriber.subscriber_id] === message.id
                                    : false
                                }
                              ...
                              <span
                                className={`absolute top-0 w-2 h-3 pointer-events-none ${isOutgoing ? "-right-2" : "-left-2"}`}
                              >
                                {isOutgoing ? (
                                  <svg viewBox="0 0 8 13" className={isDark ? "fill-[#005C4B]" : "fill-[#D9FDD3]"}>
                                    <path d="M5.188 0H0v11.193l6.467-8.625C7.526 1.156 6.958 0 5.188 0z" />
                                  </svg>
                                ) : (
                                  <svg viewBox="0 0 8 13" className={isDark ? "fill-[#202C33]" : "fill-white"}>
                                    <path d="M1.533 3.568L8 12.193V0H2.812C1.042 0 .474 1.156 1.533 2.568z" />
                                  </svg>
                                )}
                              </span>
                              ...
                              {editingMessageId === message.id ? (
                                <div className="flex flex-col min-w-[260px] max-w-[420px]">
                                  {/* Edit header bar */}
                                  <div
                                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-t-lg text-[11px] font-medium ${
                                  ...
                                  {/* Action buttons */}
                                  <div
                                    className={`flex items-center justify-between px-3 py-1.5 rounded-b-lg ${
                              ...
                              <div className="flex items-center justify-end gap-1 mt-1">
                                ...
                                {(message as any).metadata?.edited && (
                                  <span
                                    className={`text-[11px] italic ${isOutgoing ? themeClasses.messageTime : themeClasses.secondaryText}`}
                                  >
                                    editada
                                  </span>
                                )}
                                <span
                                  className={`text-[11px] ${isOutgoing ? themeClasses.messageTime : themeClasses.secondaryText}`}
                                >
              ...
              <div className={`px-4 py-2 ${themeClasses.header} border-t ${themeClasses.border}`}>
                <div className={`flex items-center gap-3 p-2 rounded-lg ${themeClasses.sidebar}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {selectedFile.type.startsWith("audio/") && (
                      <p className={`text-xs ${themeClasses.secondaryText}`}>🎧 Ouça antes de enviar</p>
                    )}
                  </div>
                  <Button
                    variant="ghost"
            ...
            {replyToMessage && (
              <div
                className={`px-4 py-2 flex items-center gap-3 border-t ${isDark ? "bg-[#1F2C34] border-[#313D45]" : "bg-[#F0F2F5] border-[#E9EDEF]"}`}
              >
                <div
                  className={`flex-1 rounded-lg px-3 py-2 border-l-4 ${replyToMessage.direcao === "saida" ? "border-l-[#00A884]" : "border-l-[#6B7B8D]"} ${isDark ? "bg-[#111B21]" : "bg-white"}`}
                >
                  <p
                    className={`text-xs font-medium ${replyToMessage.direcao === "saida" ? "text-[#00A884]" : isDark ? "text-[#E9EDEF]" : "text-[#111B21]"}`}
                  >
                ...
                <button
                  ...
                  className={`p-1 rounded-full ${isDark ? "hover:bg-white/10 text-[#8696A0]" : "hover:bg-black/5 text-[#667781]"}`}
                >
                  <X className="h-5 w-5" />
                </button>
              </div>
            ...
            {/* Input de Mensagem */}
            <div
              className={`min-h-[52px] md:h-[66px] px-2 md:px-4 py-1.5 md:py-2 flex items-center gap-1 md:gap-2 ${themeClasses.header} border-t ${isDark ? "border-[#222D34]/50" : "border-[#E9EDEF]/50"}`}
            >
              <input
                type="file"
                ref={fileInputRef}
          ...
          /* Tela vazia */
          <div
            className={`h-full flex flex-col items-center justify-center ${themeClasses.emptyState} border-b-[6px] border-[#00A884]`}
          >
            <div className="text-center max-w-md px-4">
              ...
              <h2
                className={`text-2xl font-semibold mb-3 bg-gradient-to-r from-[#00A884] to-[#008069] bg-clip-text text-transparent`}
              >
              ...
              <p className={`text-[14px] ${themeClasses.secondaryText} leading-6 mb-6`}>
                ...
                <br />
                As mensagens são atualizadas em tempo real.
              </p>
