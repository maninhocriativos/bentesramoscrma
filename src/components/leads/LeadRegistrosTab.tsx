...
import { Loader2, Plus, Calendar, Clock, NotebookPen } from 'lucide-react';
...
export function LeadRegistrosTab({ leadId }: { leadId: string }) {
  ...
  const fetchRegistros = useCallback(async () => {
    ...
    setRegistros((interacoes || []).map((i: any) => ({
      ...
      // Se quem gravou é o usuário atual, usa o nome já resolvido pelo
      // contexto de perfil (evita depender só do join, e cobre o instante
      // em que o registro acabou de ser criado nesta mesma sessão).
      responsavelNome: (i.responsavel_id && i.responsavel_id === user?.id && fullName)
        || (i.responsavel_id && perfisPorId.get(i.responsavel_id))
        || 'Equipe',
    })));
    setLoading(false);
  }, [leadId, user?.id, fullName]);
      ...
      <div className="p-5 space-y-4">
        <div className="p-3.5 rounded-xl border border-primary/20 bg-primary/5 space-y-2">
          <Label className="text-[10px] font-semibold text-primary uppercase tracking-wider flex items-center gap-1">
            <NotebookPen className="w-3 h-3" /> Novo registro
          </Label>
        ...
        {!loading && registros.length > 0 && (
          <p className="text-[10px] text-muted-foreground px-0.5">{registros.length} registro{registros.length !== 1 ? 's' : ''}</p>
        )}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-14 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">Use o campo acima pra anotar o que aconteceu com este lead</p>
          </div>
        ) : (
