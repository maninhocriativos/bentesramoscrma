            ...
            return (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div><p className="text-[11px] text-muted-foreground uppercase tracking-wider font-medium mb-1">Status</p>
                    <Badge variant="secondary" className="text-xs">{selectedTarefa.status}</Badge></div>
                  <div><p className="text-[11px] text-muted-foreground uppercase tracking-wider font-medium mb-1">Prioridade</p>
                    <span className="text-xs px-2 py-0.5 rounded-lg font-semibold" style={{ background: cfg.bg, color: cfg.color }}>{cfg.label}</span></div>
                  <div><p className="text-[11px] text-muted-foreground uppercase tracking-wider font-medium mb-1">Responsável</p>
                    <div className="flex items-center gap-1.5"><User className="h-3.5 w-3.5 text-muted-foreground" /><span className="text-sm">{responsavel?.fullName || 'Não atribuído'}</span></div></div>
                  <div><p className="text-[11px] text-muted-foreground uppercase tracking-wider font-medium mb-1">Data Limite</p>
                    <div className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5 text-muted-foreground" />
                      <span className="text-sm">{selectedTarefa.data_limite ? new Date(selectedTarefa.data_limite).toLocaleDateString('pt-BR') : 'Sem prazo'}</span></div></div>
                </div>
                {selectedTarefa.descricao && (
                  <div><p className="text-[11px] text-muted-foreground uppercase tracking-wider font-medium mb-1">Descrição</p>
                    <p className="text-sm bg-muted/30 rounded-lg p-3">{selectedTarefa.descricao}</p></div>
                )}
                {selectedTarefa.aprovacao_status && (
                  <div className={cn('rounded-lg p-3 border',
                    ...
                    {selectedTarefa.aprovacao_feedback && <p className="text-xs text-muted-foreground">{selectedTarefa.aprovacao_feedback}</p>}
                  </div>
                )}
