...
// ✅ FIX — extrai campos do processo normalizado de forma robusta
// O Escavador retorna campos em diferentes locais dependendo da versão da API
const extractFromProc = (proc: any) => ({
  ...
  // ✅ classe_cnj deve ser o NOME legível, não o código numérico
  classeCnj:     proc.classe        || proc.classeCodigo   || '',
  valorCausa:    proc.valorCausa    || proc.valor_causa    || null,
  dataAjuiz:     proc.dataAjuizamento    || proc.data_ajuizamento    || proc.data_distribuicao || '',
...
function FieldGroup({ children }: { children: React.ReactNode }) {
  return <div className="bg-muted/20 rounded-2xl p-4 space-y-3 border border-border/30">{children}</div>;
}
        ...
        <div className="flex items-center justify-between gap-2 p-2.5 pl-3">
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <CollapsibleTrigger asChild>
              <Button variant="ghost" size="icon" className="h-5 w-5 text-muted-foreground/50 hover:text-primary"><Pencil className="h-2.5 w-2.5" /></Button>
            </CollapsibleTrigger>
...
export function ProcessoModalExpanded({
  ...
  // Load data when processo changes
  useEffect(() => {
    ...
    if (processo) {
      ...
      setFormData({
        ...
        orgao_julgador:            processo.orgao_julgador || processo.vara_comarca || '',
        grau:                      processo.grau || '',
        ...
        area:                      p.area || '',
        fase:                      p.fase || '',
        // ✅ FIX — classe_cnj usa o nome legível, não o código
        classe_cnj:                processo.classe_cnj || processo.titulo_acao || '',
        assunto_cnj:               p.assunto_cnj || '',
        segredo_justica:           p.segredo_justica || false,
        // ✅ FIX — normaliza datas do banco (pode vir como YYYY-MM-DD ou ISO)
  ...
  // ✅ FIX — carrega partes do banco relacional com fallback para partes_json
  useEffect(() => {
    if (!isNew && isOpen && processo?.id && !autoFetchDone && draftHydrated) {
      setAutoFetchDone(true);
      (async () => {
        try {
          // Carregar partes da tabela relacional
          const { data: dbPartes } = await supabase
            .from('processo_partes').select('*').eq('processo_id', processo.id);
          ...
          if (dbPartes && dbPartes.length > 0) {
            ...
            // Fallback: usa partes_json e persiste na tabela relacional
            setPartes(processo.partes_json);
            await supabase.from('processo_partes').insert(
              processo.partes_json.map((p: any) => ({
          ...
          // Carregar movimentos da tabela relacional
          const { data: dbMov } = await supabase
            .from('processo_movimentacoes').select('*')
      ...
      // Auto-refresh se dados incompletos ou stale
  ...
  // Auto-fetch para processos novos ao digitar CNJ
  useEffect(() => {
  ...
  // ✅ FIX — fetchProcessoData usa extractFromProc para mapear campos corretamente
    ...
    try {
      ...
      if (data?.encontrado && data?.processo) {
        ...
        setFormData(prev => ({
          ...
          vara_comarca:      fields.orgaoJulgador || prev.vara_comarca,
          ...
          valor_causa:       fields.valorCausa?.toString() || prev.valor_causa,
  ...
  // ✅ FIX — handleRefreshStatus também usa extractFromProc
  const handleRefreshStatus = async (silent = false) => {
    ...
    try {
      ...
      if (data?.encontrado && data?.processo) {
        ...
        const newMovs   = (proc.movimentos || []).slice(0, 100);
        setPartes(newPartes);
        ...
        if (processo?.id) {
          await supabase.from('processos').update({
            titulo_acao:             fields.classe || null,
            status:                  mapStatus(proc.status),
            tribunal:                fields.tribunal || null,
            orgao_julgador:          fields.orgaoJulgador || null,
            vara_comarca:            fields.orgaoJulgador || null,
            assunto:                 fields.assunto || null,
            assunto_cnj:             fields.assuntoCnj || null,
            classe_cnj:              fields.classeCnj || null,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) {
        ...
        if (result?.error) { toast.error('Erro ao salvar', { description: (result.error as any)?.message }); return; }
        savedId = processo.id;
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="w-[96vw] max-w-[1200px] rounded-2xl max-h-[94vh] overflow-hidden flex flex-col p-0 gap-0">
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  {formData.numero_processo && (
                    <p className="text-[11px] font-mono text-muted-foreground mt-0.5">{formData.numero_processo}</p>
                  )}
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 min-h-0 flex flex-col overflow-hidden">
            <div className="px-6 pt-3 pb-0 shrink-0 bg-card border-b border-border/40">
              <TabsList className="h-9 bg-muted/50 rounded-xl p-1 gap-1">
                ...
                <TabsTrigger value="movimentos" className="rounded-lg text-xs h-7 px-4 gap-1.5 data-[state=active]:shadow-sm">
                  ...
                  {movimentos.length > 0 && (
                    <span className="ml-1 h-4 min-w-4 px-1 rounded-full bg-primary/15 text-primary text-[9px] font-black flex items-center justify-center">
                      {movimentos.length}
                    </span>
                  )}
                </TabsTrigger>
            ...
            <TabsContent value="processo" className="flex-1 min-h-0 mt-0 overflow-hidden">
              <div className="flex gap-0" style={{ height: 'calc(94vh - 230px)' }}>
                    ...
                    <div>
                      ...
                      <FieldGroup>
                        <Row2>
                          ...
                          <Field label="Assunto">
                            <button type="button" onClick={() => setAssuntoPickerOpen(true)}
                              className={`flex items-center justify-between w-full h-10 px-3 rounded-xl border text-sm transition-all ${formData.assunto ? 'bg-card border-border text-foreground' : 'bg-card border-border text-muted-foreground'} hover:border-primary/50 focus:outline-none focus:ring-2 focus:ring-ring`}>
                      ...
                      <FieldGroup>
                        <Row2>
                          <Field label="Justiça / Tribunal">
                            <Input value={formData.tribunal} onChange={e => update('tribunal', e.target.value)} className="rounded-xl bg-card h-10" placeholder="Ex: TJAM" />
                          </Field>
                        ...
                        <Row2>
                          <Field label="Vara / Comarca">
                            <Input value={formData.vara_comarca} onChange={e => update('vara_comarca', e.target.value)} className="rounded-xl bg-card h-10" placeholder="Vara" />
                          </Field>
                          <Field label="Órgão Julgador">
                            <Input value={formData.orgao_julgador} onChange={e => update('orgao_julgador', e.target.value)} className="rounded-xl bg-card h-10" placeholder="Ex: 2ª Vara Cível" />
                          </Field>
                        </Row2>
                        <Row2>
                          <Field label="Sistema Judicial">
                            <Input value={formData.sistema_judicial} onChange={e => update('sistema_judicial', e.target.value)} className="rounded-xl bg-card h-10" placeholder="Ex: PJe, e-SAJ, PROJUDI" />
                          </Field>
                          <Field label="Complemento">
                            <Input value={formData.complemento_enderecamento} onChange={e => update('complemento_enderecamento', e.target.value)} className="rounded-xl bg-card h-10" placeholder="Complemento" />
                          </Field>
                        </Row2>
                        <Row4>
                      ...
                      <FieldGroup>
                        ...
                        <Row2>
                          <Field label="Classe CNJ">
                            <Input value={formData.classe_cnj} onChange={e => update('classe_cnj', e.target.value)} className="rounded-xl bg-card h-10" placeholder="Ex: Procedimento Comum Cível" />
                          </Field>
                          <Field label="Assunto CNJ">
                            <Input value={formData.assunto_cnj} onChange={e => update('assunto_cnj', e.target.value)} className="rounded-xl bg-card h-10" placeholder="Código ou nome CNJ" />
                          </Field>
                        </Row2>
                        <label className="flex items-center gap-2 cursor-pointer">
                      ...
                      <FieldGroup>
                        <Row3>
                          <Field label="Valor da Ação (R$)">
                            <Input value={formData.valor_causa} onChange={e => update('valor_causa', e.target.value.replace(/[^0-9.,]/g, ''))} className="rounded-xl bg-card h-10" placeholder="0,00" inputMode="decimal" />
                          </Field>
                          <Field label="Valor Provisionado (R$)">
                            <Input value={formData.valor_provisionado} onChange={e => update('valor_provisionado', e.target.value.replace(/[^0-9.,]/g, ''))} className="rounded-xl bg-card h-10" placeholder="0,00" inputMode="decimal" />
                          </Field>
                ...
                <div className="px-6 py-5">
                  {movimentosEnriquecidos.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-24 gap-3 border-2 border-dashed border-border/40 rounded-2xl">
                      ...
                      <Button variant="outline" size="sm" className="mt-1 gap-2 rounded-xl h-9"
                        onClick={() => handleRefreshStatus(false)} disabled={fetchingData || !isValidCnj}>
                    ...
                    <div className="space-y-2 pb-6">
                      <p className="text-xs text-muted-foreground mb-4 font-medium">
                        {movimentosEnriquecidos.length} movimentação(ões) · clique para detalhes
                      </p>
                      {movimentosEnriquecidos.map((mov, i) => (
                        <div key={i}
                          className="group flex items-start gap-3 p-3.5 rounded-2xl border border-border/40 bg-card hover:bg-accent/30 hover:border-border cursor-pointer transition-all duration-150 hover:shadow-sm"
                          onClick={() => { setSelectedMovimento(mov); setMovModalOpen(true); }}
                        >
                          <div className="shrink-0 mt-0.5">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-bold border ${getCategoriaColor(mov.categoria)}`}>
                              {mov.badge}
                            </span>
                          </div>
                          <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between px-6 py-3.5 border-t border-border/60 bg-card shrink-0">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" className="rounded-xl gap-1.5 h-9 border-border/60"
                onClick={() => handleRefreshStatus(false)} disabled={fetchingData || !isValidCnj}>
